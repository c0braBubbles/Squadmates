...
var countstart = 0;
firebase.database().ref('/Grupper/' + key + '/Innlegg').orderByKey().limitToLast(4).on('child_added', function (snapshot) {
    countstart++;
    var owner = snapshot.child("Eier").val();
...
function getPost(lastkey) {
    var counting = 0;
    firebase.database().ref('/Grupper/' + key + '/Innlegg').orderByKey().limitToLast(4).endAt(lastkey.slice(0, -1)).on('child_added', function (snapshot) {
        counting++;
        var owner = snapshot.child("Eier").val();
        var title = snapshot.child("Tittel").val();
        var description = snapshot.child("Beskrivelse").val();
